...
public class TileEntityStabilizerTesting extends TileNodeStabilizer implements IInfusionStabiliser {
    ...
    public void setMatrix() {
        try {
            ...
            } else if (worldObj.getTileEntity(this.xCoord - 1, this.yCoord + 3, this.zCoord)
                    instanceof TileInfusionMatrix) {
                matrix = (TileInfusionMatrix) worldObj.getTileEntity(this.xCoord - 1, this.yCoord + 3, this.zCoord);
            } else if (worldObj.getTileEntity(this.xCoord, this.yCoord + 3, this.zCoord + 1)
                    instanceof TileInfusionMatrix) {
                matrix = (TileInfusionMatrix) worldObj.getTileEntity(this.xCoord, this.yCoord + 3, this.zCoord + 1);
            } else if (worldObj.getTileEntity(this.xCoord, this.yCoord + 3, this.zCoord - 1)
                    instanceof TileInfusionMatrix) {
                matrix = (TileInfusionMatrix) worldObj.getTileEntity(this.xCoord, this.yCoord + 3, this.zCoord - 1);
            }
